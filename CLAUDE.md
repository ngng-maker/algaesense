...
- **Agent-generated LED control profiles** (ramps, sinusoidal cycles, etc., per JAXSR's DoE recommendation) — user's original stated intent was Hermes generating a Python control script per-experiment that runs on the Pi. This is a genuinely open, safety-relevant design fork (arbitrary LLM-generated code executing on hardware vs. the LLM only supplying parameters for a small, fixed set of trusted profile shapes our own code executes) — confirm the execution model explicitly with the user before writing any of the execution engine, not just the bounds-check re-validation detail already agreed. Not started.
