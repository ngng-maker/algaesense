...
def _live_readings() -> None:
    ...
    with voc_col:
        ...
        if voc_rows:
            ...
            df = pd.DataFrame(voc_rows)[["timestamp", "pid_voltage_mv"]].set_index("timestamp")
            st.line_chart(df)
        else:
    ...
    with camera_col:
        ...
        if camera_rows:
            # Only rows that actually carry a feature vector can be
            # scored -- a capture that failed partway through might not.
            greenness_series = [
                {"timestamp": row["timestamp"], "greenness": _greenness(row["image_feature_vector"])}
                for row in camera_rows
                if row.get("image_feature_vector")
            ]
            if greenness_series:
                latest_greenness = greenness_series[-1]["greenness"]
                st.metric("Latest greenness (2G-R-B)", f"{latest_greenness:.1f}")
                df = pd.DataFrame(greenness_series).set_index("timestamp")
                st.line_chart(df)
            else:
