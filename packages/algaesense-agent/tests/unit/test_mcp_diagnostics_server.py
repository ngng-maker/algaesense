...
async def test_ambient_baseline_tool_persists_the_covariate_model_when_asked(tmp_path: Path, monkeypatch) -> None:
    """The fix for discover_led_response_dynamics's missing ambient-baseline
    correction: run_ambient_baseline_check must be able to save its fitted
    models to disk (via persist_run_id) so a later call can reuse them."""
    monkeypatch.setenv("ALGAESENSE_DATA_DIR", str(tmp_path))
    ...
    result = await server_module.mcp.call_tool(
        "run_ambient_baseline_check", {"experiment_id": "exp_ambient", "persist_run_id": "test_ambient_run"}
    )
    payload = _payload(result)

    assert payload["persisted_as"] == "test_ambient_run"

    from jaxsr_calibration.processing.covariate import load_covariate_models

    loaded = load_covariate_models(
        "test_ambient_run", tmp_path / "derived" / "diagnostics" / "ambient_baseline"
    )
    assert loaded["PID01"].method == "ols"
    assert loaded["PID01"].beta_rh == payload["covariate_models"]["PID01"]["beta_rh"]
